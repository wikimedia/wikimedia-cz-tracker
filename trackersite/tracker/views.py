...
def export(request):
    if request.method == 'POST':
        ...
        if typ == 'ticket':
            ...
            for ticket in tickets:
                response.writerow([ticket.id, ticket.created, ticket.updated, ticket.event_date, ticket.event_url, ticket.name, ticket.requested_by(), ticket.topic.grant.full_name, ticket.topic.name, str(ticket.subtopic), ticket.state_str(), ticket.deposit, ticket.description, ticket.mandatory_report, ticket.accepted_expeditures(), ticket.preexpeditures()['amount'], ticket.expeditures()['amount']])
            return response
        elif typ == 'grant':
